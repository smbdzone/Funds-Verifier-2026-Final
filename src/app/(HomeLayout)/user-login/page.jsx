...
const page = () => {
  ...
  // Set for OTP-gated roles (Evaluator, Sub-Evaluator, ...) after the password step.
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        captureRedirectQueryParam(searchParams)
        const data = await login(values, router)
        if (data?.otpRequired) {
          setOtpChallenge({
      ...
      <div className='p-5 flex flex-col gap-20 justify-center items-center bg-prussianBlue h-full text-white'>
        ...
        {otpChallenge ? (
          <LoginOtpStep
            ...
            resendInSeconds={otpChallenge.resendInSeconds}
            onVerify={(otp) =>
              verifyLoginOtp({ email: otpChallenge.email, otp }, router)
            }
            onResend={() => resendLoginOtp(otpChallenge.email)}
            onBack={exitOtpStep}
          />
        ) : (
          <form
          ...
          >
            ...
            <input
              ...
              className={`w-full bg-transparent rounded-full border ${formik.touched.password && formik.errors.password
                ? 'border-red-500'
                : 'border-white'
                } py-2 px-4`}
