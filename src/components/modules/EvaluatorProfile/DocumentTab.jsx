...
export const DocumentTab = () => {
  ...
  const handleFileChange = async (e) => {
    ...
    if (!isPdf) {
      toast.error('Please upload a PDF only (same as verification-certificate API).')
      return
    }
        ...
        <div className='rounded custom-shadow md:mx-10 mx-5 pb-3'>
          <Disclosure as='div' className={`disclosure`} defaultOpen={true}>
            {({ open }) => (
              <>
                <Disclosure.Button
                  className={`w-full primary-gradient rounded py-3 md:px-7 px-3 gap-4 justify-between items-center flex ${open && 'mb-3'
                    }`}
                >
