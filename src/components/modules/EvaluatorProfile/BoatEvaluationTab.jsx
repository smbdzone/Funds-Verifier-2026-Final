...
export const BoatEvaluationTab = () => {
    ...
    try {
      ...
      const responses = await Promise.allSettled(
        parentIds.map((parentId) =>
          customAxios.get(
            `${process.env.NEXT_PUBLIC_BASE_URL}/evaluator/parent/${parentId}`
          )
        )
      )
      ...
      console.error('Error fetching sub evaluators:', error)
      // alert('Failed to fetch evaluators')
    }
  }

  const handleShowCertificate = async (boatId) => {
    try {
      const response = await customAxios.get(`/boat/${boatId}`)
      const certificateUrl = getListingDocumentSrc(
      ...
      if (certificateUrl) {
        ...
        alert('No evaluation certificate found for this boat.')
      }
    } catch (error) {
      console.error('Error fetching boat certificate:', error)
  ...
  const handleAssignEvaluator = async (boatId, evaluatorId) => {
    try {
      ...
      await customAxios.post(`/assets/assign?userId=${userId}`, requestData)
                                      ...
                                      <td className='py-3 px-4'>
                                        ...
                                        >
                                          ...
                                          <button
                                            ...
                                            onClick={() => {
                                              router.push(
                                                `/evaluator-profile/boat-evaluation/${property.uuid}`,
                                              )
                                              closeActionMenu()
                                            }}
                                            className={evaluationMenuItemClass}
