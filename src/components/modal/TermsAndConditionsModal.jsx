    ...
    if (consentData?.consentTaxTerms && consentData.consentTerms) {
      try {
        const res = await customAxios.put(
          ...
          { electronicConsent: consentData, userState: 'active' }
        )
        if (res?.status === 200) {
          onClose()
          setUser(res?.data)
  ...
  return (
    <AnimatePresence>
      ...
      <div className='fixed inset-0 bg-black/10 bg-opacity-30 flex justify-center items-center z-50 p-4'>
        <motion.div
          className='relative bg-white shadow- mt-5 rounded-lg py-10 md:px-8 w-[350px] overflow-y-auto overflow-hidden lg:w-[800px] md:w-[700px] h-[80%]'
                ...
                <div className='custom-shadow rounded flex flex-col gap-2'>
                  <>
                    <div className='custom-shadow rounded'>
                      <div className='btn-gradient rounded md:px-12 px-4 overflow-x-auto'>
                        <nav
                      ...
                      user?.userState === 'active' ? (
                        ...
                        <>
                          ...
                          <div className='flex justify-end gap-3 md:me-10 my-3'>
                            <button
                              onClick={handleSave}
                              className='flex text-white justify-center btn-gradient border-0 py-2 md:px-10 px-4 focus:outline-none text-lg font-medium rounded'
                            >
                              Accept
                            </button>
                          </div>
