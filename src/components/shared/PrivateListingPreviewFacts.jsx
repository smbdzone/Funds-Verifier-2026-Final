...
export default function PrivateListingPreviewFacts({ listing, className = '' }) {
  ...
  const typeFact = facts.find((fact) => fact.key === 'type')
  const rest = facts.filter((fact) => fact.key !== 'type')
  const threeCols = rest.length === 3
  ...
  return (
    ...
    >
      ...
      {typeFact ? (
        <span className='block border-b border-[#D7C590]/25 bg-[#00253f] px-3 py-2.5 text-center sm:px-3.5'>
          ...
          <span className='mt-1 block truncate text-[13px] font-semibold leading-snug text-[#F3E6B8] sm:text-[14px]'>
            {typeFact.value}
          </span>
        </span>
      ) : null}
