...
const FileUpload = ({
  ...
  const isQr = type === 'qrScan' || /qr/i.test(String(label || ''))
  const [singlePreviewUrl, setSinglePreviewUrl] = useState('')

  useEffect(() => {
    if (type === 'pictures' || type === 'video') {
      setSinglePreviewUrl('')
      return undefined
    }
        ...
        {type !== 'pictures' &&
          ...
          (files instanceof File || files instanceof Blob) && (
            <div className='fit h-full overflow-hidden p-2 relative group'>
              {type === 'video' ? (
              ...
              ) : isQr && singlePreviewUrl ? (
                <QrDecodeHover src={singlePreviewUrl}>
                  <Image
                    width={150}
                    height={100}
                ...
                <Image
                  width={150}
                  height={100}
                  src={singlePreviewUrl || URL.createObjectURL(files)}
                  alt={`upload-${type}`}
                />
              )}
              <button
