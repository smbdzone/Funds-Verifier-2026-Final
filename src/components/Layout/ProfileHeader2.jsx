...
import SellerProfileSidebar from '../Sidebar/SellerProfileSidebar'
import NotificationDropdown from '../Buttons/Notifications'

const ProfileHeader2 = () => {
  const { user, logout, switchUserRole, isSwitchingRole } = useProfile()
    ...
    `}
      ...
      {isSwitchingRole && (
        <div className='fixed inset-0 bg-white bg-opacity-70 flex flex-col items-center justify-center z-[9999]'>
          <div className='animate-spin rounded-full h-10 w-10 border-4 border-prussianBlue border-t-transparent mb-3'></div>
          <p className='text-prussianBlue text-sm font-medium'>
            Switching role...
          </p>
        </div>
      )}
